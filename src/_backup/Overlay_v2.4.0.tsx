// BACKUP of src/Overlay.tsx as of v2.4.0
// Saved before overlay rewrite — do not edit this file.
    ...
    <div className={`ov-card${isUnknown ? " ov-card-unknown" : ""}`} style={{ left, width }}>
      ...
      <div className="ov-price-row">
        <span className="ov-price-plat">
          {item.plat != null ? <><PlatIcon size={14} />&nbsp;{item.plat}</> : <span className="ov-price-na">—</span>}
        </span>
        {item.vaulted && <span className="ov-lock">🔒</span>}
        <span className="ov-price-ducat">
          {item.ducats != null && item.ducats > 0 ? <><DucatIcon size={14} />&nbsp;{item.ducats}</> : <span className="ov-price-na">—</span>}
        </span>
      </div>
      {hasSet && <>
        ...
        <div className="ov-comp-grid">
          {(item.components ?? []).slice(0, 4).map(c => {
            ...
            const stripped = raw !== 'Blueprint' && raw.endsWith(' Blueprint') ? raw.slice(0, -' Blueprint'.length) : raw;
...
export default function Overlay() {
  ...
  const craftingRef  = useRef<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    const processPayload = (paths: string[], positions: number[]) => {
      ...
      const base: RewardItem[] = paths.map((path, i) => {
        if (path.startsWith("?:")) {
          return { unique_name: path + "-" + i, raw_unique: "", slot_x: positions[i] ?? 0.5, name: path.slice(2), category: "Unrecognized" };
        }
        const lk   = path.replace("/Lotus/StoreItems/", "/Lotus/");
        const meta = byUnique[lk] ?? byUnique[path];
        return { unique_name: path + "-" + i, raw_unique: lk, slot_x: positions[i] ?? 0.5, name: meta?.name ?? path.split("/").pop() ?? path, category: meta?.category, vaulted: meta?.vaulted, ducats: meta?.ducats, owned_qty: qty[lk] ?? qty[path] ?? 0 };
      });
      setRewards(base);
      paths.forEach(async (path, i) => {
        ...
        const setEntry = Object.values(cat).find(item => item.name === setName && item.category !== 'Blueprints' && item.category !== 'Parts');
        if (setEntry) {
          const recipe = await invoke<RC[]>("get_recipe", { unique_name: setEntry.unique_name }).catch(() => []);
          if (recipe.length) {
            const parts = recipe.filter(c => { const clk = c.unique_name.replace("/Lotus/StoreItems/", "/Lotus/"); const cm = cat[clk] ?? cat[c.unique_name]; return cm?.category === 'Parts' || cm?.category === 'Blueprints'; });
            if (parts.length >= 1) {
              const liveQty = quantRef.current;
              const liveCraft = craftingRef.current;
              components = parts.map(c => { const clk = c.unique_name.replace("/Lotus/StoreItems/", "/Lotus/"); const cm = cat[clk] ?? cat[c.unique_name]; return { unique_name: c.unique_name, name: c.name, needed: c.count ?? 1, owned: resolveOwnedFn(c.unique_name, c.name, cat, liveQty, liveCraft), ducats: cm?.ducats }; });
            }
          }
        }
        if (!components) {
          const liveQty = quantRef.current;
          const liveCraft = craftingRef.current;
          const parts = Object.values(cat).filter((item: any) => item.name?.startsWith(setPrefix) && (item.category === 'Parts' || item.category === 'Blueprints')).map((item: any) => ({ unique_name: item.unique_name, name: item.name as string, needed: 1, owned: resolveOwnedFn(item.unique_name, item.name as string, cat, liveQty, liveCraft), ducats: item.ducats as number | undefined }));
          if (parts.length >= 2) components = parts;
        }
        if (!components) return;
        const liveQty = quantRef.current;
        const compSets = components.length > 0 ? Math.floor(Math.min(...components.map(c => c.owned / c.needed))) : 0;
        const builtLk = (setEntry?.unique_name ?? '').replace("/Lotus/StoreItems/", "/Lotus/");
        const builtQty = setEntry ? (liveQty[builtLk] ?? liveQty[setEntry.unique_name] ?? 0) : 0;
        const completeSets = builtQty > 0 ? Math.max(compSets, 1) : compSets;
        setRewards(prev => prev.map((r, idx) => idx === i ? { ...r, set_name: setName, components: components!, complete_sets: completeSets, missing_plat: 0, total_plat: 0 } : r));
        await Promise.all(components.map(async comp => {
          ...
          setRewards(prev => prev.map((r, idx) => {
            ...
            const comps = r.components.map(c => c.unique_name === comp.unique_name ? { ...c, plat } : c);
            const missing_plat = comps.reduce((acc, c) => { if (c.owned < c.needed && c.plat) return acc + c.plat * (c.needed - c.owned); return acc; }, 0);
            const total_plat = comps.reduce((acc, c) => c.plat != null ? acc + c.plat * (c.needed ?? 1) : acc, 0);
    ...
    const unsubTrigger = listen<null>("relic-trigger", () => { setRewards([]); prevKey.current = ""; });
    const unsub = listen<{ items: string[]; positions: number[] } | null>("relic-rewards", async (e) => {
      const payload = e.payload;
      if (!payload || payload.items.length === 0) { setRewards([]); prevKey.current = ""; getCurrentWebviewWindow().close().catch(() => {}); return; }
      if (dataReady) processPayload(payload.items, payload.positions);
      else pendingEvent = { paths: payload.items, positions: payload.positions };
    });

    invoke<{ items: string[]; positions: number[] } | null>("get_pending_relic_rewards")
      .then(pending => { if (pending && pending.items.length > 0) { if (dataReady) processPayload(pending.items, pending.positions); else pendingEvent = { paths: pending.items, positions: pending.positions }; } })
      .catch(() => {});

    Promise.allSettled([invoke<any[]>("get_all_items"), invoke<Record<string, number>>("get_current_quantities"), invoke<CraftingJobTs[]>("get_current_crafting")]).then(([itemsR, quantitiesR, craftingR]) => {
      if (itemsR.status === 'fulfilled') { const byUnique: Record<string, any> = {}; for (const i of itemsR.value) byUnique[i.unique_name] = i; catalogRef.current = byUnique; }
      if (quantitiesR.status === 'fulfilled') quantRef.current = quantitiesR.value;
      if (craftingR.status === 'fulfilled') { const byCraft: Record<string, number> = {}; for (const job of craftingR.value) { const lk = job.unique_name.replace("/Lotus/StoreItems/", "/Lotus/"); byCraft[lk] = (byCraft[lk] ?? 0) + 1; } craftingRef.current = byCraft; }
      dataReady = true;
      if (pendingEvent) { processPayload(pendingEvent.paths, pendingEvent.positions); pendingEvent = null; }
    });
    ...
    const unsubInv = listen<{ quantities: Record<string, number> }>("inventory-update", (e) => {
      ...
      setRewards(prev => prev.map(r => {
        ...
        const cat = catalogRef.current;
        const updatedComponents = r.components.map(c => ({ ...c, owned: resolveOwnedFn(c.unique_name, c.name, cat, newQty, craftingRef.current) }));
        const compSets = updatedComponents.length > 0 ? Math.floor(Math.min(...updatedComponents.map(c => c.owned / c.needed))) : 0;
        const setEntry = Object.values(cat).find((item: any) => item.name === r.set_name && item.category !== 'Blueprints' && item.category !== 'Parts') as any;
        const builtLk = setEntry ? (setEntry.unique_name as string).replace("/Lotus/StoreItems/", "/Lotus/") : '';
        const builtQty = setEntry ? (newQty[builtLk] ?? newQty[setEntry.unique_name] ?? 0) : 0;
        const completeSets = builtQty > 0 ? Math.max(compSets, 1) : compSets;
  ...
  if (rewards.length === 0) return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(10,10,10,0.80)', borderBottom: '2px solid #f85149', padding: '8px 16px', color: '#f85149', fontSize: 13, fontFamily: 'monospace', zIndex: 99999, pointerEvents: 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
      <span>FrameForge — waiting for relic rewards…</span>
      <span style={{ color: '#8b949e', fontSize: 11 }}>{getCurrentWebviewWindow().label}</span>
    </div>
  );
